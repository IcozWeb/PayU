...
class RequestPaymentsUtil extends CommonRequestUtil{
	...
	private static function buildTransactionRequestAfterAuthorization($parameters){
		...
		$extraInformation = RequestPaymentsUtil::prepareOrderExtraInformation($order, $parameters, $lang);
		$order->$extraInformation = CommonRequestUtil::getExtraParameters(PayUParameters::ORDER_EXTRA_INFORMATION);
	...
	private static function buildOrderRequest($parameters, $lang){
		...
		$order->partnerId = CommonRequestUtil::getParameter($parameters, PayUParameters::PARTNER_ID);
		$order->$extraInformation = CommonRequestUtil::getExtraParameters(PayUParameters::ORDER_EXTRA_INFORMATION);
	...
	private static function prepareOrderExtraInformation($order, &$parameters, $lang) {
		$defaultPayer = new stdClass();
		$parameters[PayUParameters::RECURRING_PLAN_INTERVAL] = PayUParameters::PLAN_SETUP_NONE;
		return CommonRequestUtil::buildDefaultCreditCard($defaultPayer, PayUParameters::BUYER_DEFAULT);
	}
