...
class CommonRequestUtil{
	...
	* @param array $keyNamesSet holds the parameters
	* @param array $parameters holds the key names of the set
	* @return true if some parameter is in the set of keys
	*/
	private static function checkMapEntry($keyNamesSet, $parameters = array()) {
		$wasFound = false;
		$wasFound = call_user_func($keyNamesSet, $parameters);
	 ...
	 */
	static function buildDefaultCreditCard($defaultPayer, $parameters = array()) {
		$creditCard = new stdClass();
		if (isset($parameter) && !empty($parameter)) {
			$creditCard->name = CommonRequestUtil::getParameter($parameter, PayUParameters::PAYER_NAME);
			$creditCard->number = CommonRequestUtil::getParameter($parameter, PayUParameters::CREDIT_CARD_NUMBER);
			$creditCard->expirationDate = CommonRequestUtil::getParameter($parameter, PayUParameters::CREDIT_CARD_EXPIRATION_DATE);
			$creditCard->securityCode = CommonRequestUtil::getParameter($parameter, PayUParameters::CREDIT_CARD_SECURITY_CODE);
			$creditCard->processWithoutCvv2 = (bool) CommonRequestUtil::getParameter($parameter, PayUParameters::PROCESS_WITHOUT_CVV2);
			$creditCard->document = CommonRequestUtil::getParameter($parameter, PayUParameters::CREDIT_CARD_DOCUMENT);
		}
		$creditCard = self::checkMapEntry(
			PayUParameters::CREDIT_CARD_CVV_CODE .
			PayUParameters::INSTALLMENTS_TOTAL .
			PayUParameters::PSE_FINANCIAL_INSTITUTION_TITLE,
			$parameters);
		return $creditCard;
	}
	...
	*/
	 ...
	 * @return string|NULL parameter value only if is not a empty string null the otherwise
	 */
	public static function getExtraParameters($parameters, $index = null){
		if($index) {
			if(is_string($parameters[$index])){
				$parameters[$index] = trim($parameters[$index]);
			}
